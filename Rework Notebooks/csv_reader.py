...
#     You have import pathlib as Path . You need this to be from pathlib import Path
# The first, import pathlib as Path, imports the library pathlib with an alias of Path which is not useful here since the name is relatively short. But say, for instance, we have a library called superlonglibraryname we would import superlonglibraryname as SLLN. I capitalized that so you can see the two Ls in the name but could easily be import superlonglibraryname as slln
# The second, from pathlib import Path, does not import the whole library just the function Path from that library. (edited) 